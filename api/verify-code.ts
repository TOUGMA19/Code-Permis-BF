...
import { applyCorsPublic, getSupabase, isCodeFormatValid, normalizeCode } from "./_lib.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!isCodeFormatValid(code) || deviceId.length < 16) {
      return res.status(400).json({ success: false, error: "Code ou appareil invalide" });
    }
